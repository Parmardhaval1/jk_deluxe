...
class ToysController extends GetxController {
  ...
  Future<void> deleteLastTicket() async {
    ...
    try {
      final response = await http.post(
        ...
        body: json.encode({'username': username.value, 'game': 'toy'}),
        headers: {'Content-Type': 'application/json'},
      );
  ...
  Future<void> _checkWins() => checkAndAnnounceWins(
        historyEndpoint: 'Application/toy_history.php',
        gameKey: 'toy',
        username: username.value,
        box: box,
      );
  ...
  Future<void> fetchItems() async {
    try {
      final response = await http
          .get(Uri.parse(Api.getUrl('Application/get_items.php?game=toy')));
